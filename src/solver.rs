    ...
    T: Number,
{
    ...
    fn sum_weights(weights: &Vec<T>) -> T {
        weights.iter().fold(T::zero(), |sum, i| sum + i.clone())
    }
     ...
     * - step size: `10e-3`
     *
     * (Note that these values will be converted to `T` via
     * `T::from_str_radix()`.)
     */
    pub fn new(
        time_interval: &[T; 2],
    ...
    /**
     * Select the desired algorithm to solve the givem problem, also updating
     * the weights based on default values. To manually alter these values,
     * please check `Self::change_weight()`.
     */
     ...
     * calling `Self::validate()` within an `assert!`. See that function for
     * more details.
     */
